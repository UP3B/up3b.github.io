...
map
  .getView()
  .fit(
    [12311917.099563, -910771.814222, 12458085.60754, -840493.587437],
    map.getSize()
  );
      ...
      if (typeof clusteredFeatures !== "undefined") {
        if (doPopup) {
          for (var n = 0; n < clusteredFeatures.length; n++) {
            ...
            popupText += "<li><table>";
            popupText +=
              "<a>" + "<b>" + layer.get("popuplayertitle") + "</b>" + "</a>";
      ...
      if (currentFeature) {
        ...
        if (
          ...
          if (typeof clusteredFeatures == "undefined") {
            ...
            radius =
              parseFloat(featureStyle.split("radius")[1].split(" ")[1]) +
              clusterLenght;
          }
  ...
  for (var i = 0; i < wms_layers.length; i++) {
    if (wms_layers[i][1] && wms_layers[i][0].getVisible()) {
      var url = wms_layers[i][0]
        .getSource()
        .getFeatureInfoUrl(evt.coordinate, viewResolution, viewProjection, {
          INFO_FORMAT: "text/html",
        });
      if (url) {
        const wmsTitle = wms_layers[i][0].get("popuplayertitle");
        var ldsRoller =
          '<div id="lds-roller"><img class="lds-roller-img" style="height: 25px; width: 25px;"></img></div>';
...
document.getElementsByClassName("search-layer-input-search")[0].placeholder =
  "Search feature ...";
...
attributionList.innerHTML = `2024 © UP3B BBWS Bengawan Solo   
`;
...
var attributionControl =
  document.getElementsByClassName("bottom-attribution")[0];
